...
void CCalibrationView::GetExtrinsicParameters()
{
	...
	*/
	...
	GetClientRect(rect);
	CDC *pDC = GetDC();
	pDC->FillSolidRect(CRect(rect.left, rect.top, rect.right, rect.bottom),RGB(0, 0, 255)); 
	...
	for(int i=0; i<SPOT_EXTR_HEIGHT_NUM; i++)			//	行
		...
		{
			int gridx = nFullWidth/(SPOT_EXTR_WIDTH_NUM-1);
			int gridy = nFullHeight/(SPOT_EXTR_HEIGHT_NUM-1);
			
			ObjPoints[i*SPOT_EXTR_WIDTH_NUM+j].x = gridx*(j+0);
			ObjPoints[i*SPOT_EXTR_WIDTH_NUM+j].y = gridy*(i+0);
...
*/
		...
		// 手动外参标定 2012-6-3 Night	
		// 从StdPoints.xml文件中读外参标定点
		
	const char* in_filename = "c:\\Ext.xml";
	CvMat* StdPoints = cvCreateMat(SPOT_EXTR_HEIGHT_NUM*SPOT_EXTR_WIDTH_NUM,2,CV_32F);
	CvFileStorage* fs = cvOpenFileStorage( in_filename, NULL, CV_STORAGE_READ);
	StdPoints = (CvMat*) cvReadByName(fs, 0, "std_matrix");
	cvReleaseFileStorage(&fs);//释放存储的数据
	int cnt_t=0;
	for(int i=0;i<SPOT_EXTR_HEIGHT_NUM;i++)
		for(int j=0;j<SPOT_EXTR_WIDTH_NUM; j++)
		{
			ImagePoints[i*SPOT_EXTR_WIDTH_NUM+j].x=(int)(StdPoints->data.fl[cnt_t++]);
			ImagePoints[i*SPOT_EXTR_WIDTH_NUM+j].y=(int)(StdPoints->data.fl[cnt_t++]);
		}
	...
	// 输出标定信息
	fs = cvOpenFileStorage("c:\\cfg.xml", 0, CV_STORAGE_WRITE);
