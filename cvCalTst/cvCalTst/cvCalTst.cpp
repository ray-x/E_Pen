...
void read_camera_params( void)

{
	CvFileStorage* fs = cvOpenFileStorage( "..\\camera.yml", 0, CV_STORAGE_READ );

	if (!fs)
		return;
	CvFileNode* node = cvGetFileNodeByName(fs,NULL,"image_count");
	
	int i=cvReadInt(node,0);

	node = cvGetFileNodeByName(fs,NULL,"aspect_ratio");
	float j=cvReadInt(node,0);

	node = cvGetFileNodeByName(fs,NULL,"distortion_coefficients");
	CvMat *distMat= cvCreateMat( 1, 4, CV_32F );
	//cvReadByName(fs, node, "distortion_coefficients");
	distMat=  (CvMat *) cvRead (fs, node);
	//distMat=cvRead(fs,disMat)
...
int main(int argc, char* argv[], char * envp[])
{
    ...
    double _camera[9], _dist_coeffs[5];
    CvMat camera = cvMat( 3, 3, CV_64F, _camera );
    CvMat dist_coeffs = cvMat( 1, 5, CV_64F, _dist_coeffs );
    CvMat *extr_params = 0, *reproj_errs = 0;
    double avg_reproj_err = 0;
    int mode = DETECTION;
    int undistort_image = 1;
    CvSize img_size = {0,0};
    ...
    for(;;)
    {
        ...
        if(mode == CALIBRATED && undistort_image )
        {
			...
			string s1=imagename;
			string outFilename = s1+"_undistor.jpg";
			cvSaveImage(outFilename.c_str(), view);
            cvReleaseImage( &t );
        }

        cvShowImage( "Image View", view );
        key = cvWaitKey(capture ? 1000 : 3000);
